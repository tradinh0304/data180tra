...
date: '`r format(Sys.Date(), "09/23/2023")`' # no need to change date since this line grabs today's date
output: 
  html_document
---
20/20. Good job. 
...
```{r}
pie(table(Cars93$Type),main="Frequency off Distribution of Cars Type",cex=1.2)
```
...
```{r}
hist(Cars93$Horsepower, cex.lab = 1.2, cex.axis = 1.2, col = "lightblue", xlab = "Horsepower")
```
...
```{r}
hist(Cars93$Horsepower, cex.lab = 1.2, cex.axis = 1.2, col = "lightblue",breaks=seq(0,300,25), xlab = "Horsepower")
```
...
```
The histogram doesn't show any break 
